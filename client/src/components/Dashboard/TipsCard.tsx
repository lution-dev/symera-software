...
const TipsCard: React.FC<TipsCardProps> = ({ tipSetIndex: forcedTipSetIndex, isCreatingFirstEvent }) => {
  ...
  const handleCloseCard = (e: React.MouseEvent) => {
    ...
    localStorage.setItem(STORAGE_KEY_SHOW, 'false');
  };

  const CardVisuals = (
    <div className="group relative overflow-hidden rounded-xl border border-primary/20 bg-gradient-to-r from-primary/10 via-background to-background p-3 sm:p-4 shadow-sm hover:shadow-md transition-all cursor-pointer">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[11px] font-medium text-primary hover:underline flex items-center gap-1 mt-0.5">
        ...
        <button
          ...
          className="h-8 w-8 shrink-0 flex items-center justify-center rounded-full text-muted-foreground hover:bg-muted transition-colors z-10"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
  ...
  const ModalContent = () => (
    <div className="space-y-4 py-4">
      {activeTipSet.modalTips.map((tip, idx) => (
        <div key={idx} className={`${isMobile ? 'flex gap-4 p-4 rounded-2xl bg-white/5 border border-white/5' : 'flex gap-4 p-3 rounded-xl bg-white/5 border border-white/5 text-left'}`}>
